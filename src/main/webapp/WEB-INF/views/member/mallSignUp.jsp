...
<link rel="stylesheet" href="${pageContext.request.contextPath}/resources/css/member/signup.css" />
 <jsp:include page="/WEB-INF/views/common/header.jsp">
 	<jsp:param value="입점몰 회원가입" name="title"/>
 </jsp:include>
    <div id="signup-normal">
		<div class="signup-normal-wrapper">
			<div class="signup-cont">
				<h3 class="top-title">입점몰 회원가입</h3>
					...
					<div class="signup-form-wrapper" >
						<form name="signupFrm" id="signupFrm" action="${pageContext.request.contextPath}/member/memberEnroll?${_csrf.parameterName}=${_csrf.token}" method="POST" enctype="multipart/form-data">
							<div class="necessary-info">
								...
								<div class="info-wrapper">
									<div class="label-wrapper">
										<label for="memberName">업체 대표명</label><span class="neccesary-star">*</span>
									</div>
									<input type="text" id="memberName" name="memberName" class="input-cont" placeholder="대표 수의사 입력(한글)" required />
								</div>
								<div class="info-wrapper-with-msg">
									<div class="inner-wrapper">
										<div class="label-wrapper">
											<label for="nickname">업체명</label><span class="neccesary-star">*</span>
										</div>
										<input type="text" id="nickname" name="nickname" class="input-cont" placeholder="병원명 입력(2~15자)" required />
										<input type="hidden" id="nickValid" value="0" />
									</div>
									<div class="msg-wrapper msg-nickname">
										<div class="void-space">&nbsp;</div>
										<span class="nickname-result ok">사용가능한 업체명입니다.</span>
										<span class="nickname-result fail">중복되는 업체명입니다.<br>업체명 앞에 지역명을 붙여보세요.</span>
										
									</div>
								</div>
								<div class="info-wrapper">
									<div class="label-wrapper">
										<label for="phone">업체 대표번호</label><span class="neccesary-star">*</span>
									</div>
									<input type="tel" id="phone" name="phone" class="input-cont"  placeholder="업체 대표번호 입력(- 없이)"  required/>
								</div>
			                    ...
			                    <div class="info-wrapper">
				                    <div class="label-wrapper">
										<label for="file">사업자 등록증</label><span class="neccesary-star">*</span>
				                    </div>
									<span id="file-box">  
						...
						<input type="hidden" name="memberType" value="hospital" />
						<input type="hidden" name="gradeNo" value="97" />
 ...
 */
 ...
 frm.addEventListener('submit', (e) => {
	    ...
	    //3.대표 수의사명 검사 : 한글2글자 이상만 허용. 
	    // 한글 검사
	    if(!/^[가-힣]{2,}$/.test(memberNameVal)){
	        alert('대표 수의사명에는 2글자 이상의 한글만 사용할 수 있습니다.');
	        return false;
	    }
	    
	    //4. 업체명 검사 : 한글2글자 이상만 허용. 
	    // 한글 검사
	    if(!/^[가-힣\s]{2,15}$/.test(memberNicknameVal)){
	        alert('업체명에는 2글자 이상 15글자 이하의 한글만 사용할 수 있습니다.');
	        return false;
	    }

	    //5. 전화번호 검사
	    
	    if(!(/^(01[016789]{1}|02|0[3-9]{1}[0-9]{1})-?[0-9]{3,4}-?[0-9]{4}$/.test(memberPhoneVal))){
