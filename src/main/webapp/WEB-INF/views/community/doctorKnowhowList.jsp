  ...
  <div id="Pmypage-submenu" class="submenu">
    <ul id="sub">
		...
		<tr>
			...
			<c:forEach items="${list}" var="communityDknowBoard" varStatus="vs">
				<tr data-no="${communityDknowBoard.dknowbNo}">
					<td>${communityDknowBoard.dknowbNo}</td>
					<td>${communityDknowBoard.dknowbTitle}</td>
					<td>${communityDknowBoard.memberId}</td>
					<td>${communityDknowBoard.regDate}</td>
					<td>${communityDknowBoard.readCount}</td>
				</tr>
			</c:forEach>
		</table>
		<%-- 	<nav>${pagebar}</nav> --%>
		<button type="button" id="btn-add-free" class="btn_free">
			<a href="${pageContext.request.contextPath}/community/doctorKhowEnroll">글쓰기
			</a>
		</button>
</section>
